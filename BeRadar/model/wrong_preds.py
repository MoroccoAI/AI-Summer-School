...
def test(classifier, backbone, loader, args): 
    ...
    with torch.no_grad(): # stop gradient updating
        ...
        for idx, (data, targets, indices) in enumerate(loader): 
            ...
            negatives = indices[targets==0]
            negatives_predictions = winners[negatives]
            false_positives = negatives[negatives_predictions==1]
            fp_list.append(false_positives.cpu())
            # get indices of false positives: 
    accuracy = (tp+tn)/total_count
    precision = tp/(tp+fp)
    recall = tp/(tp+fn)
    fp_list = torch.cat(fp_list)
    return accuracy, precision, recall, fp_list
...
if __name__=="__main__":
    ...
    # test model
    if args.val_shots>0:
        val_accuracy, val_precision, val_recall, fp_list_val = test(classifier, backbone, val_loader, args) 
    else: 
        val_accuracy, val_precision, val_recall = 0, 0, 0
    accuracy, precision, recall, fp_list_test = test(classifier, backbone, test_loader, args)
    train_size, val_size, test_size  = len(data_train), len(data_val), len(data_test)
    print('list of false positives: ')
    print([test_dataset.data[int(idx.item())] for idx in fp_list_test])
